...
function Main() {
  ...
  return (
    <>
      ...
      <div
        className={clsx([
          'w-full backdrop-blur fixed bg-primary/50 z-[60] border-b border-white/[0.08] -mt-5 -mx-3 sm:-mx-8 mb-6 dark:bg-darkmode-800/50 md:hidden shadow-lg',
          "before:content-[''] before:w-full before:h-screen before:z-10 before:fixed before:inset-x-0 before:bg-black/30 before:backdrop-blur before:transition-opacity before:duration-200 before:ease-in-out",
          !activeMobileMenu && 'before:invisible before:opacity-0',
          activeMobileMenu && 'before:visible before:opacity-100',
        ])}
      >
        <div className="h-[70px] px-3 sm:px-8 flex items-center shadow-2xl">
          <a href="/" className="flex mr-auto">
            <img alt="React Vite Dashboard" className="w-6" src={logoUrl} />
            <span className="ml-3 text-lg text-white">React Dashboard</span>
          </a>
        ...
        <div
          ref={scrollableRef}
          className={clsx([
            'h-screen z-20 top-0 left-0 w-[270px] -ml-[100%] bg-primary/80 shadow-xl backdrop-blur-sm transition-all duration-300 ease-in-out dark:bg-darkmode-800',
        ...
        >
          ...
          >
            <Lucide
              icon="XCircle"
              className="w-8 h-8 text-white transform -rotate-90 mr-4"
              onClick={() => {
                setActiveMobileMenu(!activeMobileMenu)
              }}
            />
